...
public class RepositoryGenerator {
    ...
    public String contenu(String classUser, String token){
        String cont = "";
            if(token == classUser && token != ""){
                cont += "\t @Query (\"\"\" \n"
                        + "\t select t " + capitalizeFirstLetter(token) + " t inner join " + capitalizeFirstLetter(classUser) + " u on t." 
                        + classUser + ".id = u.id \n" + "\t where t." + classUser + ".id = u.id = :" + classUser + "Id and t.loggedOut = false \n"
                        + "\t \"\"\")\n\n";
                
                cont += "\t List<" + capitalizeFirstLetter(token) +"> findAllTokensBy" + capitalizeFirstLetter(classUser) +"(Integer userId); \n\n";
                cont += "\t Optional<" +  capitalizeFirstLetter(token) + "> findBy" + capitalizeFirstLetter(token) + "(String " + token + "); \n\n";
            }

        return cont;
    }

    public String contet (String token, String classUser) {
        String cont = "";
            if(token == classUser && classUser != ""){
                cont += "\t Optional<" +  capitalizeFirstLetter(token) + "> findByUsername(String username); \n\n";
            }
        return cont;
    }
    ...
    public void generate(String className,String classUser ,String Token) throws Exception {
        try {
            ...
            for(int i =0 ; i< classes.size(); i++){
                ...
                className = RepositoryGenerator.capitalizeFirstLetter(classes.get(i));
                String template = loadTemplateFromFile();

                String contenus = this.contenu(classes.get(i),Token);
                String contets = this.contet(classes.get(i), classUser);
                String pack = this.pack();
                template = template.replace("##PACKAGE##", pack);

                String imports = this.importe(classes.get(i));
                ...
                String kilasy = this.getName(classes.get(i));
                template = template.replace("#CLASSNAME#", kilasy);

                String exten = this.extend();
                template = template.replace("##EXTENDS##", exten);

                template = template.replace("#CLASS_NAME#", classes.get(i));
                template = template.replace("#CLASS_NAME_2#", classes.get(i).toLowerCase());
                ...
                template = template.replace("#CONTENTES#", contenus);
                template = template.replace("#CONTENTE2#", contets);
