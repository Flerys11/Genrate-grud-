    ...
    public String getAllValueToForms(String tableName, Connection connection) throws Exception {
        ...
        try {
            for (int i = 0 ; i < attributs.length; i++) {
                ...
                allForms += "\t\t<input type=\""+ this.getValue("type", attributs[i].getType()) + "\" className=\"form-control\" id=\"" + attributs[i].getNom() + "\" name=\"" + attributs[i].getNom() + "\" placeholder=\"" + attributs[i].getNom() + "\" value={formData." + attributs[i].getNom() + " || \'\'} onChange={handleChange} />\n";
                allForms += "\t</div>\n";
            }
            for (int i = 0 ; i < attributsFK.length; i++) {
                if(attributsFK[i].getNom() != null) {
                    ...
                    allForms += "\t<div className=\"form-group\">\n";
                    allForms += "\t\t<label for=\"" + attributsFK[i].getNom() + "\">" + capitalizeFirstLetter(attributsFK[i].getType()) + "</label>\n";
                    allForms += "\t\t<select className=\"form-control\" id=\"" + attributsFK[i].getNom() + "\" name=\"" + attributsFK[i].getNom() + "\" value={formData." + attributsFK[i].getNom() + " || \'\'} onChange={handleChange} >\n";
                    allForms += "\t\t{" + attributsFK[i].getType() + ".map((item) => (\n";
                    allForms += "\t\t\t<option  key={item." + attributPK.getNom() +"} value={item." + attributPK.getNom() + "}>" + "{item." + attribut.getNom() + "}</option>\n";
                    allForms += "\t\t))}\n";
                    allForms += "\t\t</select>\n";
                    allForms += "\t</div>";
                }
        ...
        try {
            for (int i = 0 ; i < attributsFK.length; i++) {
                if(attributsFK[i].getNom() != null) {
                    ...
                    allForms += "\t" + ".then((data) => {" + "\n";
                    allForms += "\t" + "\t" + "if(data.content.length > 0) {" + "\n";
                    allForms += "\t" + "\t" + "\t" + "const objectKeys = Object.keys(data.content[0])" + "\n";
                    allForms += "\t" + "\t" + "\t" + "setColumns(objectKeys)" + "\n";
                    allForms += "\t" + "\t" + "\t" + "set" + this.capitalizeFirstLetter(attributsFK[i].getType()) + "(data.content)" + "\n";
                    allForms += "\t" + "\t" + "}" + "\n";
                    allForms += "\t" + "})" + "\n";
